...
class Coronagraph:
    def __init__(self, dir, verbose=True):
        ...
        # set above
        self.set_position_angle(0.0 * u.deg)
        ...
        # interpolate planet data depending on how
        if "1" in self.type:
            if "x" in self.type:
                xy_index = 0
            elif "y" in self.type:
            ...
            self.ln_offax_psf_interp = interp1d(
                self.offax_psf_offset_list[:, xy_index],
    ...
    def set_position_angle(self, position_angle=0.0 * u.deg):
        """
        Sets the position angle (of what?)
        Args:
        ...
        """
        # deg
        self.position_angle = self.offax_psf_base_angle - position_angle
